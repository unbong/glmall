...
import com.atguitu.gulimall.gulimallsearch.feign.AttrFeign;
import com.atguitu.gulimall.gulimallsearch.service.MallSearchService;
import com.atguitu.gulimall.gulimallsearch.vo.AttrRespVo;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            esResult = rest.search(searchRequest, GulimallElasticSearchConfig.COMMON_OPTIONS);
        }
        catch (IOException e)
        {
            e.printStackTrace();
        ...
        result = buildSearchResult(esResult, param);
     ...
     * build result
     *
     * @param searchResponse, searchParam
     * @return
     */
    private SearchResult buildSearchResult(SearchResponse searchResponse, SearchParam searchParam ) {
        ...
        //2.3 总页码
        Integer totalPages = (int)total % EsConstant.PAGE_SIZE == 0 ?
        ...
        if ( searchParam.getAttrs() != null && searchParam.getAttrs().size()> 0)
        {
            ...
            List<SearchResult.NavVo> navs =searchParam.getAttrs().stream().map(
                    attr->{
                        SearchResult.NavVo nav = new SearchResult.NavVo();
                        String[] split = attr.split("_");

                        try{
                            R r =attrFeign.info(Long.parseLong( split[0]));

                            if(r.getCode() == 0){
                                AttrRespVo attrVo = r.getData("attr", new TypeReference<AttrRespVo>(){});
                                nav.setNavName(attrVo.getCatelogName());
                            }
                        }
                        catch (Exception e)
                        {
                            log.error("get attr feing failed");
...
//                        编码的转换处理没有加
                        ...
                        nav.setLink("http://search.gulimall.com/search.html" + (replace.isEmpty()?"":"?"+replace));

                        nav.setNavValue(split[1]);
                        return nav;
    ...
    private SearchRequest buildSearchRequest(SearchParam searchParam) {
        ...
        //3. 分页
        searchSourceBuilder.from((searchParam.getPageNum() - 1) * EsConstant.PAGE_SIZE);
        ...
        SearchRequest request = new SearchRequest(new String[]{EsConstant.ELASTIC_INDEX_PRODUCT}, searchSourceBuilder);
